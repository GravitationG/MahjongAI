...
def get_MyMJPolicyGradient(path, **kwargs):
	if path not in loaded_models:
		try:
			print("Load Model")
			loaded_models[path] = MyMJPolicyGradient.load(path)
		except Exception as e:
			print(e)
			loaded_models[path] = MyMJPolicyGradient(**kwargs)
	return loaded_models[path]

class MyMJPolicyGradient:
	...
	def learn(self, display_cost = True):
		...
		if len(self.__ep_obs)>0:
			...
			araa = np.array(self.__ep_rs)
			self.__invalide = 0.99*self.__invalide + 0.01 *np.sum(araa==-5)/np.ma.size(araa)
			#print(self.__invalide)
		...
		if display_cost:
			print("#%4d: %.4f, invalide: %.4f"%(self.__learn_step_counter + 1, loss,self.__invalide))
