...
from MLUtils import get_MyMJPolicyGradient
...
class MyPGGenerator(MoveGenerator):
	def __init__(self, player_name, pg_model_path, is_train, skip_history = False, display_tgboard = False, display_step = False):
		...
		self.clear_history()
		self.model = get_MyMJPolicyGradient(pg_model_path)
	...
	def update_transition(self, state_, reward = 0, action_filter_ = None):
		...
		self.model.store_transition(self.pg_model_history["state"], self.pg_model_history["action"], reward, self.pg_model_history["action_filter"])
	...
	def decide_chow(self, player, new_tile, choices, neighbors, game):
		...
		action, value = self.model.choose_action(state, action_filter = action_filter, return_value = True, strict_filter = not self.is_train)
