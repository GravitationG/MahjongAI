...
from MLUtils import get_ActorCritic
...
class ACGenerator(MoveGenerator):
	...
	def update_history(self, state, action, action_filter):
		...
		self.ac_model_history["state"] = state
		self.ac_model_history["action"] = action
		self.ac_model_history["action_filter"] = action_filter
	...
	def update_transition(self, state_, reward = 0, action_filter_ = None):
		...
		if type(state_) == str and state_ == "terminal":
			state_ = self.ac_model_history["state"]	
			
			
		self.history_waiting = False
		
		td_error = self.critic.learn(self.ac_model_history["state"],reward,state_)
		
		#print(td_error)
		loss = self.actor.learn(self.ac_model_history["state"],self.ac_model_history["action"],td_error[0][0],self.ac_model_history["action_filter"])
		

		'''
		self.actor.store_transition(self.ac_model_history["state"], self.ac_model_history["action"], reward, self.ac_model_history["action_filter"])
		'''

	def clear_history(self):
		self.history_waiting = False
		self.ac_model_history = {
			"state": None,
			"action": None,
			"action_filter": None
		}
	...
	def decide_chow(self, player, new_tile, choices, neighbors, game):
		...
		action, value = self.actor.choose_action(state, action_filter = action_filter, return_value = True, strict_filter = not self.is_train)
